function [data targetVec] = GenerateGaussianDataMulti(samples)
...
label1 = repmat([1; 0; 0] , 1, samples);

label11 = repmat([1; 0; 0] , 1, samples);
disp(cov(data1'));
...
label2 = repmat([0; 1; 0] , 1, samples);

label22 = repmat([2; 0; 0] , 1, samples);
disp(cov(data2'));

% extra one?
data3 =  chol(Sigma3) * randn(2, samples) + repmat(Mean3,1,samples);
% generate target vectors for class 1
label3 = repmat([0; 0; 1] , 1, samples);

label33 = repmat([3; 0; 0] , 1, samples);

disp(cov(data3'));

% concatenate class0 and class1 datasets into single dataset
data = [data1 data2 data3];
targetVec = [label1 label2 label3 ];

targetVec2 = [label11 label22 label33 ];
